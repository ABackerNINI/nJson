...
#define _NJSON_SET_IF_NOT_DEFAULT(ROOT, K, M)             \
    ...
    json_object_set_value(json_value_get_object(ROOT), K, \
                          _NJSON_SERIALIZE_FUNC(M))
...
#define _NJSON_GET(ROOT, K, M, T)                              \
    ...
    if (T) _NJSON_DESERIALIZE_FUNC(T, M)

#define _NJSON_SERIALIZE_SUPER_CLASS(SUPER, ROOT) \
    njson_value_move(_NJSON_SERIALIZE_FUNC(*(SUPER)), ROOT)
#define _NJSON_DESERIALIZE_SUPER_CLASS(SUPER, ROOT) \
    _NJSON_DESERIALIZE_FUNC(ROOT, SUPER)
...
inline size_t njson_value_move(JSON_Value *src_njson_val,
                               JSON_Value *des_njson_val) {
    if (src_njson_val == NULL || des_njson_val == NULL) {
        return 0;
    }

    if (src_njson_val->type != JSONObject ||
        des_njson_val->type != JSONObject) {
        return 0;
    }
