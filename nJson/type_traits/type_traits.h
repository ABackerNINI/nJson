...
//
// Codes are copied from gnu std library header 'type_traits' and modified,
// since there are no type_traits for c++98.
//
#pragma once
 ...
 * Note 2:Some features can't be supported, they are listed below:
 *  constexpr;
...
struct integral_constant {  // convenient template for integral constant types
    ...
    operator __CONSTEXPR value_type() const __NOEXCEPT { return (value); }

    __CONSTEXPR value_type
    operator()() const __NOEXCEPT {  // return stored value
        return (value);
    }
};
...
template <bool _Val>
struct _cat_base
    : integral_constant<bool, _Val> {  // base class for type predicates
};
...
struct remove_cv {  // remove top level const and volatile qualifiers
    typedef
        typename remove_const<typename remove_volatile<_Ty>::type>::type type;
};
...
template <class _Ty>
struct _Add_pointer<_Ty, typename _void_t<typename remove_reference<_Ty>::type
                                              *>::type> {  // add pointer
...
struct _is_integral_helper<bool>
    : true_type {  // determine whether _Ty is integral
};
...
template <>
struct _is_integral_helper<signed short>
    : true_type {  // determine whether _Ty is integral
};
...
template <>
struct _is_integral_helper<signed int>
    : true_type {  // determine whether _Ty is integral
};
...
template <>
struct _is_integral_helper<signed long>
    : true_type {  // determine whether _Ty is integral
};

// No char16_t and char32_t before c++11
//    template<>
//    struct _is_integral_helper<char16_t> : true_type {    // determine whether
//    _Ty is integral
//    };
//
//    template<>
//    struct _is_integral_helper<char32_t> : true_type {    // determine whether
//    _Ty is integral
//    };
...
template <class _Ty>
struct is_integral
    : _is_integral_helper<typename remove_cv<_Ty>::type> {  // determine whether
                                                            // _Ty is integral
};

// TEMPLATE CLASS _is_floating_point_helper
template <class _Ty>
struct _is_floating_point_helper
    : false_type {  // determine whether _Ty is floating point
};

template <>
struct _is_floating_point_helper<float>
    : true_type {  // determine whether _Ty is floating point
};

template <>
struct _is_floating_point_helper<double>
    : true_type {  // determine whether _Ty is floating point
};

template <>
struct _is_floating_point_helper<long double>
    : true_type {  // determine whether _Ty is floating point
};
...
struct is_floating_point
    : _is_floating_point_helper<typename remove_cv<
          _Ty>::type> {  // determine whether _Ty is floating point
};
...
struct is_arithmetic
    : _cat_base<is_integral<_Ty>::value ||
                is_floating_point<_Ty>::value> {  // determine whether _Ty is an
                                                  // arithmetic type
};
...
template <class _Ty>
struct is_void : _is_void<typename remove_cv<_Ty>::type> {  // determine whether
                                                            // _Ty is void
};
...
struct _is_signed_base {  // determine whether integral _Ty is a signed or
                          // unsigned type
...
template <class _Ty>
struct is_signed
    : _is_signed_base<_Ty>::_Signed {  // determine whether _Ty is a signed type
};

// TEMPLATE CLASS is_unsigned
template <class _Ty>
struct is_unsigned : _is_signed_base<_Ty>::_Unsigned {  // determine whether _Ty
                                                        // is an unsigned type
};
}  // namespace std
