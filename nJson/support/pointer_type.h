...
/*
    This file is to support the serialization and deserialization of pointer
    type.

    Note that you need to implement your own functions to support pointers
    point to arrays. See 'cstr_type.h' for reference.
*/

template <typename T>
struct njson_support<T *> {
    static bool is_default(const T *njson_var) {
        return njson_var == NJSON_DEFAULT_VALUE_POINTER ||
               njson_support_is_default(*njson_var);
    }
