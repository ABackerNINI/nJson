...
/*set_key_value*/
template<typename _T_KEY,typename _T_VAL>
	   void set_key_value(JSON_Object *obj,const char *key,const std::map<_T_KEY,_T_VAL> &val){
		   if(key!=NULL){
		   ...
		   for(it = val.begin();it!=val.end();++it){
			   set_key_value(obj,to_string(it->first).c_str(),it->second);
		   }
	   }
template<typename _T_KEY,typename _T_VAL>
	   void set_key_value(JSON_Array *arr,const std::map<_T_KEY,_T_VAL> &val){
		   JSON_Value *doc = json_value_init_object();
		   JSON_Object *obj = json_value_get_object(doc);
		   set_key_value(obj,NULL,val);
		   json_array_append_value(arr,doc);
	   }

/*get_value*/
template<typename _T_KEY,typename _T_VAL>
	   void get_value(JSON_Object *obj,const char *key,std::map<_T_KEY,_T_VAL> *val){
...
template<typename _T_KEY,typename _T_VAL>
	   void get_value(JSON_Array *arr,size_t index,std::map<_T_KEY,_T_VAL> *val){
	   		size_t n = json_array_get_count(arr);
	   		JSON_Object *obj = json_array_get_object(arr,index);
   			get_value(obj,NULL,val);
	   }
